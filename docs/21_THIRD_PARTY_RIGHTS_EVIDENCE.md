...
작성·공식 자료 확인일: 2026-09-13 · 대상: Android `com.tteumsae.app` 0.13.0 / versionCode 32
...
[카카오 지도 SDK 로고 표시 정책](https://apis.map.kakao.com/android_v2/docs/getting-started/precautions/)은
기본 지도 로고의 변경·지속적인 가림을 허용하지 않고, 필요하면 로고 위치를 옮기도록 안내한다.
코드는 SDK 지도를 사용하며 로고를 제거하는 호출은 찾지 못했다. 지도에는 `setPadding`이
적용돼 있다. QA 담당자는 2026-09-13 결과 지도 캡처 `tourist-07-map.png`에서 오른쪽 하단
...
아래에 보관한다. 홈·검색 오버레이와 code 32 최종 APK의 전체 화면 확인은 별도다.
...
- 결과 상세 및 저장 장소 상세에 출처 문구와 공식 개방 페이지의 이용조건 링크를 연결했다.
  ...
  전체 구도와 출처 노출을 확인했다(2026-09-13). 모든 사진의 개별 권리 검증을 뜻하지 않는다.
...
오프라인 자산 기반 구현이며, 이 검증을 모든 네이티브 구성의 독립적인 전체 감사로 해석하지 않는다.
...
| 항목 | 확인된 현상 | 필요한 후속 조치 |
|---|---|---|
| 사진 출처의 완결성 | 최신 소스는 메타데이터 유무와 관계없이 상세 사진의 TourAPI 출처를 표시하며, 상세 2곳에서 공식 이용조건 링크를 제공함. code 32 실기기에서 출처 노출 확인 | 개별 사진의 원출처·유형을 계속 대조. 일반 안내 문구가 미확인 사진의 권리 허락을 대신하지 않음. 없는 작가명·연도를 만들어 넣지 않음. |
...
| 지도 로고 | 기존 결과 지도 캡처에서 카카오 로고 노출 확인, `setPadding` 적용 | 홈·검색 오버레이와 code 32 최종 APK 화면은 별도 확인. |
| OSS·폰트 고지 | code 32에 110개 모듈 목록, 원문 7개 및 오프라인 고지 화면 추가. 계약 테스트 2/2 및 최종 APK 실기기 목록·원문 열기·뒤로 흐름 통과 | 제공사 공통 고지를 내부 네이티브 구성의 독립 SBOM으로 해석하지 않으며, Kotlin 재패키징 코드의 별도 상위 NOTICE 전수 확인 등 한계는 docs22에 명시. |
...
표시와 출처 문구를 검증했다. QA 담당자는 이후 code 32 통합 검증에서 Android 단위 테스트
190개 실패 0개, lint 오류 0개·경고 48개 및 실기기 라이선스·사진 흐름 통과를 확인했다.
이 결과도 전체 사진의 개별 권리 검토 완료를 의미하지 않는다.
