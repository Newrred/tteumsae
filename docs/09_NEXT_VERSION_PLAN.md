...
3. [ ] 도보 모드의 카카오 실제 도보 경로 API 전환 가능 여부·쿼터·응답 계약 확정
4. [ ] 관광지 혼잡 예측을 DB에 선동기화하고 상세에 보조 신호로 제공
5. [ ] 도착 시각 기준 기상청 단기예보를 야외 장소에 한해 제공
6. [ ] `KorWithService2` 접근성 정보를 선택 필터와 상세에 제공
7. [ ] 전국주차장 표준데이터를 `주변 공영주차장`으로 분리 제공
8. [ ] 체류시간·연관관광지·두루누비는 Gate 3 사용자 근거가 생긴 뒤 재평가

1번 코드는 로컬 자동 검증까지 완료했다. 2번의 운영 Supabase migration과 Vercel
Production 승격은 별도 운영 변경이므로 승인 전에는 실행하지 않는다.
