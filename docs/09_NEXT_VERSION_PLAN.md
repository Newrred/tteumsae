...
5. [ ] 혼잡 API 활용신청, 별도 키 등록, 수동 동기화와 매칭률 검수 후 Cron 예약
6. [x] 도착 시각 기준 기상청 단기예보·격자 캐시·야외 장소 상세 표시 코드 구현
7. [ ] 단기예보 활용신청, 별도 키 등록, Preview 실응답 후 기능 플래그 활성화 판단
8. [x] `KorWithService2` 강원 목록·상세 동기화와 조건부 `접근성 안내` 코드 구현
9. [ ] 무장애 API 활용신청, migration 010 적용, Preview 매칭률·실기기 상세 검증
10. [ ] 실제 정보 보유 장소 비율과 사용자 필요가 확인되면 기존 입력 화면 안의 선택 필터 검토
11. [x] 전국주차장 표준데이터를 `주변 공영주차장`으로 분리 제공하는 서버·Android 코드 구현
12. [ ] 주차장 API 활용신청, migration 011, Preview 전체 순회·실기기 상세 검증
13. [ ] 체류시간·연관관광지·두루누비는 Gate 3 사용자 근거가 생긴 뒤 재평가

상세·혼잡·날씨 코드는 운영 배포됐고 migration 007~009도 적용됐다. 접근성과 주변
공영주차장은 코드만 준비됐으며 migration 010~011과 새 공급자 키는 운영에 적용하지 않았다.
날씨 기능 활성화와 혼잡·접근성·주차장 Cron 예약은 실데이터 검증 뒤 별도 운영 변경으로 진행한다.
...
도보 경로로 오인하지 않도록 직선거리로 표시하고, 운영 적재 전에는 화면에 나타나지 않는다.

운영 배포 직후 기준선은 활성 1,713곳, intro 239곳, common 9곳, media·info 각 0곳이다.
새 배포 전 마지막 presentation 실행은 media 5건 실패로 `partial`이었으므로 다음 예약 실행에서
새 코드 기준 수치와 실패 원인을 먼저 확인한다.
