...
7. [ ] 도착 시각 기준 기상청 단기예보를 야외 장소에 한해 제공
8. [ ] `KorWithService2` 접근성 정보를 선택 필터와 상세에 제공
9. [ ] 전국주차장 표준데이터를 `주변 공영주차장`으로 분리 제공
10. [ ] 체류시간·연관관광지·두루누비는 Gate 3 사용자 근거가 생긴 뒤 재평가

상세·도보·혼잡 코드는 로컬 자동 검증까지 완료했다. 운영 Supabase migration, 새 외부 키,
도보 기능 활성화, 혼잡 Cron 예약과 Vercel Production 승격은 별도 운영 변경이므로 승인 전에는
실행하지 않는다.
