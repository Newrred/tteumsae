...
- 현재 자동 검증: Android 143/143, Backend 172/172, lint 오류 0
...
- 도착 마감 인라인 휠, 단계형 로딩 모달, 결과 바텀시트와 상세 정보 정제 UI 개선 완료
- 카테고리 핀, 의미 단위 묶음 해제, 선택 카드 자동 정렬과 실제 TourAPI 상세정보 적용 완료
- 조직 소유 업로드 키·서명 AAB·release APK 핵심 경로 회귀와 카카오 업로드 키 해시 등록 완료
- release Kakao·Google OAuth 계정 동의·복귀와 프로세스 재시작 후 세션 복원 완료
- release 알림 스위치·권한·채널과 UI 출발 시각 기준 5분 전 AlarmManager 등록 완료
- 업로드 키 독립 보안 백업·Play App Signing 키 해시·실제 예약 만료 시각 알림 관찰은 미완료
...
- [x] Release-signed Google OAuth 계정 동의·앱 복귀·프로세스 재시작 세션 복원 회귀
...
- [x] 추천 핀은 카테고리 아이콘으로 구분하고 추가 이동시간은 결과 목록에 표시
- [x] 결과는 한 곳 선택을 기본 완료 경로로 제공
- [x] 선택 후 최대 체류시간·출발 권장시각·첫 구간·추가 이동시간을 위계화해 표시
...
3. 등록된 release 알림이 실제 예약 만료 시각에 표시되는지 장시간 실기기 관찰한다.
...
## 공간 정보 보강 순서 (2026-09-07 확정)

추천·지도 성능을 해치지 않도록 상세 정보, 경로 정확도, 실시간 맥락, 편의정보 순으로
붙인다. 세부 근거와 활성화 조건은 [공간 정보 보강 로드맵](14_SPATIAL_DATA_ENRICHMENT_ROADMAP.md)을
따른다.

1. [x] TourAPI `detailInfo2` 정규화와 사진별 공공누리 유형 보존·상세 화면 표시 코드
2. [ ] migration 007 적용, Preview 보강 smoke, Production 승격과 다음 예약 실행 확인
3. [x] 도보 모드의 카카오 실제 도보 경로 API 계약·쿼터 확정과 feature-gated 서버 구현
4. [ ] Vercel Preview에서 도보 권한·실응답·사용량 계측 확인 후 운영 활성화 판단
5. [x] 관광지 혼잡 예측의 선동기화·보수적 장소 매칭·상세 보조 신호 코드 구현
6. [ ] 혼잡 API 활용신청, migration 008, 수동 Preview 동기화와 매칭률 검수 후 Cron 예약
7. [x] 도착 시각 기준 기상청 단기예보·격자 캐시·야외 장소 상세 표시 코드 구현
8. [ ] 단기예보 활용신청, migration 009, Preview 실응답 후 기능 플래그 활성화 판단
9. [ ] `KorWithService2` 접근성 정보를 선택 필터와 상세에 제공
10. [ ] 전국주차장 표준데이터를 `주변 공영주차장`으로 분리 제공
11. [ ] 체류시간·연관관광지·두루누비는 Gate 3 사용자 근거가 생긴 뒤 재평가

상세·도보·혼잡·날씨 코드는 로컬 자동 검증까지 완료했다. 운영 Supabase migration, 새 외부 키,
도보·날씨 기능 활성화, 혼잡 Cron 예약과 Vercel Production 승격은 별도 운영 변경이므로
승인 전에는 실행하지 않는다.
