...
9. [ ] 무장애 API 활용 승인, 실데이터 매칭률·실기기 상세 검증 (migration 010 적용 완료)
10. [ ] 실제 정보 보유 장소 비율과 사용자 필요가 확인되면 기존 입력 화면 안의 선택 필터 검토
11. [x] 전국주차장 표준데이터를 `주변 공영주차장`으로 분리 제공하는 서버·Android 코드 구현
12. [ ] 주차장 API 활용신청·전용 키, 전체 순회·실기기 상세 검증 (migration 011 적용 완료)
13. [ ] 체류시간·연관관광지·두루누비는 Gate 3 사용자 근거가 생긴 뒤 재평가

상세·혼잡·날씨·접근성·주변 공영주차장 코드는 운영 배포됐고 migration 007~011도 적용됐다.
접근성과 주변 공영주차장 실데이터는 아직 적재하지 않았고 공영주차장 전용 키도 없다.
