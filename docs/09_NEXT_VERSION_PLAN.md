...
4. [x] 도보 모드의 카카오 실제 도보 경로 API 계약·쿼터 확정과 feature-gated 서버 구현
5. [ ] Vercel Preview에서 도보 권한·실응답·사용량 계측 확인 후 운영 활성화 판단
6. [x] 관광지 혼잡 예측의 선동기화·보수적 장소 매칭·상세 보조 신호 코드 구현
7. [ ] 혼잡 API 활용신청, 별도 키 등록, 수동 동기화와 매칭률 검수 후 Cron 예약
8. [x] 도착 시각 기준 기상청 단기예보·격자 캐시·야외 장소 상세 표시 코드 구현
9. [ ] 단기예보 활용신청, 별도 키 등록, Preview 실응답 후 기능 플래그 활성화 판단
10. [ ] `KorWithService2` 접근성 정보를 선택 필터와 상세에 제공
11. [ ] 전국주차장 표준데이터를 `주변 공영주차장`으로 분리 제공
12. [ ] 체류시간·연관관광지·두루누비는 Gate 3 사용자 근거가 생긴 뒤 재평가

상세·도보·혼잡·날씨 코드는 운영 배포됐고 migration 007~009도 적용됐다. 새 외부 키,
도보·날씨 기능 활성화, 혼잡 Cron 예약은 실데이터 검증 뒤 별도 운영 변경으로 진행한다.
