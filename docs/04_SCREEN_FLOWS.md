...
- 홈 Kakao 지도·GPS, 장소 탐색 탭과 설정 탭은 기존 동작을 유지한다.
...
도착 마감은 진입 시 현재부터 1시간 뒤를 기본값으로 잡는다. 화면 안의 24시간제 시·분
휠을 스크롤하면 별도 확인 단계 없이 절대 epoch에 즉시 반영되고, 선택값 위에는 `오늘`
또는 `내일`을 명시한다. 현재부터 15분 이상 24시간 이하만 허용하며 자정이 지난 시각은
다음 날로 해석한다.
...
- 출발지·목적지 검색은 2자 이상, 350ms debounce 후 최대 10개 결과를 표시한다.
- 출발지는 전국을 허용한다. 목적지는 사용자의 원문 검색어를 유지하되 강원 주소를
  먼저 표시하고, 강원 밖 결과에는 `현재 지원 지역 아님`을 붙여 선택 대신 지원 범위를
  안내한다. 주소를 판별할 수 없는 결과와 최종 진행 시에는 좌표 행정구역을 재검증한다.
...
- 지도 위의 스크림과 흰 카드로 입력·결과 화면과 로딩 레이어를 구분한다.
- 세 진행 항목은 현재 항목의 로딩에서 완료 체크로 순차 전환한다.
- 응답이 준비되면 `틈새를 찾았어요` 완료 상태를 짧게 보여준 뒤 RESULTS로 이동한다.
...
- 추천 목록은 지도 위에서 드래그 가능한 바텀시트로 열고 접을 수 있다.
- 상단에는 마지막 계산 시각과 수동 재확인 아이콘 하나만 표시한다.
- 선택 후 `여기서 최대 약 N분 머물 수 있어요`와 `H시 M분까지 출발`을 표시한다.
- 선택형 출발 알림은 선택 카드 안에서 출발 권장 시각과 함께 표시한다.
...
- 추천 장소 이미지·이름·주소·태그·운영상태를 보여주며 주소는 이름 바로 아래에 둔다.
- 공백, `null`, 문자열 `"null"`은 방문 정보에서 제거하고 실제 값이나 명시적 태그만
  노출한다. 값이 하나도 없으면 한 번의 확인 안내만 표시한다.
- 분 단위 값은 `N시간 M분`처럼 읽기 쉬운 형태로 표시한다.
