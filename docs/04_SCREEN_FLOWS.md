...
활성 흐름에는 시간 입력 화면이 없다. 홈에서 탐색을 시작할 때 내부 상태를 `deadlineMinutes=1440`, `safetyBufferMinutes=15`, `transport=CAR`, `mode=ON_THE_WAY`로 설정한다. 따라서 현재 결과는 사용자가 입력한 도착 마감에 대한 **시간 보장 결과가 아니라 출발→목적 기본 경로 주변 후보 탐색**이다.

`AppScreen.TIME`, `TimeScreen`, `SearchMode.NEARBY`, `ModeSelector`는 소스에 남아 있지만 위 전이에서는 도달할 수 없다.

## 2. 전역 상태와 복원 범위

| 상태 | 저장 방식 | 사용 화면 | 복원 한계 |
|---|---|---|---|
| `screen`, 이름, 내부 시간, 이동수단 | `rememberSaveable` 일부 | 전체 | 화면 enum은 복원돼도 다음 좌표·추천과 불일치할 수 있음 |
...
| 액션 | 상태 전이·효과 |
|---|---|
| 검색 카드 | `ON_THE_WAY`, `CAR`, 1440분, 15분 버퍼로 초기화하고 LOCATION |
...
클릭 후 목적지를 `/api/region`으로 확인한다. 강원도 밖이면 `목적지가 강원도 밖이라 이동 중 들를 장소를 추천할 수 없어요.` 토스트 후 그대로 남는다. 성공하면 이름/좌표를 확정하고 **TIME을 건너뛰어 CONDITIONS로 이동**한다.
...
Android 요청값은 현재 고정적으로 `ON_THE_WAY`, `CAR`, `deadlineMinutes=1440`, `safetyBufferMinutes=15`다. 서버는 baseRoute path 주변 800m~8km corridor에서 후보를 찾는다.
...
현재 진행 문구의 `남은 시간`, `도착 전 여유 시간`은 사용자 시간 입력이 없는 새 흐름과 맞지 않는다. 다음 문구 개정에서는 `기본 경로와 주변 장소를 계산하고 있어요`처럼 바꿔야 한다.

## 7. RESULTS — 경로 주변 후보와 0~5 경유지
...
flowchart TD
    ...
    E --> F["POST /api/route"]
    F -->|"성공"| G["Kakao 주행시간·거리·통행료·path 갱신"]
    F -->|"실패"| H["토스트 + ESTIMATE fallback"]
    G --> I["선택 확정·배지·상단 카드 갱신"]
    H --> I
```

- 경유 순서는 자동 최적화가 아니라 사용자가 추가한 순서다.
- `/api/route`는 0~5개를 허용하고 출발→각 waypoint→최종 목적지를 Kakao Mobility로 계산한다.
...
- 하단 CTA는 선택 0곳에서도 활성: 직행 카카오맵 URL.
...
5. 장소 소개와 한 줄 주소 링크.
6. 고정 bottom bar: 15분 여유 안내, 56dp 카카오맵 CTA.
...
- 하트: 현재 장소 로컬 저장/해제.
- 주소 행: 카카오맵 장소명 검색.
- 하단 CTA: **현재 상세 장소 한 곳만** 경유지로 넣고 출발→장소→최종 목적지를 연다.
- 결과 화면에서 고른 다른 경유지는 상세 CTA에 포함하지 않는다.
- `목적지에 늦지 않도록 15분의 여유를 남겨요`는 내부 고정 버퍼를 사용자 마감처럼 표현해 부정확하다. `경로 주변 추천의 기본 여유값은 15분이에요` 등으로 바꾸거나, 실제 시간 입력을 복원한 뒤 사용해야 한다.
- 상단 뒤로: RESULTS.
