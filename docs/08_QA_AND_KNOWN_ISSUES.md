...
- [ ] 목적지 원문 검색에서 최대 10건이 보이고 강원 결과가 먼저 배치된다.
- [ ] 강원 밖 목적지는 `현재 지원 지역 아님`과 지속 안내를 표시하며 선택되지 않는다.
...
- [x] 공식 전국주차장정보표준데이터의 HTTPS 요청 주소와 관리번호·구분·유형·주소·운영시간·
  요금·좌표·장애인 전용 주차구역·기준일 필드 계약을 대조했다.
- [x] 강원 공영·유효 좌표만 정규화하고 독립 lease·페이지 cursor로 upsert하는
  `stage=parking`을 구현했다.
- [x] migration 011은 원문 테이블 RLS를 켜고 public·anon·authenticated 권한을 회수해
  service role만 접근하도록 제한한다.
- [x] 장소 단건 API는 반경 1km 안의 가까운 3곳만 직선거리 기준으로 반환하고, 테이블이
  아직 없어도 기존 상세 200을 유지한다.
- [x] Android 추천 상세와 저장 상세는 값이 있을 때만 `주변 공영주차장`을 표시하며
...
## 17. 2026-09-08 공간 정보 실데이터 운영 검증

- [x] 환경변수 등록과 health 200: 혼잡도·주차장·기상청 설정 `true`, 날씨 기능 활성화.
- [x] migration 012 적용 뒤 공급자 제약에 `KMA`, `PUBLIC_DATA`가 포함되고 예약 RPC는
  `service_role`과 `postgres`만 실행 가능함을 확인했다.
- [x] 관광지 집중률 4페이지·2,880건 전체 동기화, exact unique name 450건 연결,
  예측 범위 `2026-09-08`~`2026-10-07` 확인.
- [x] 소돌해수욕장 단건 API에서 당일 29.37%, `한산 예상`, 상대 예측 근거를 확인했다.
- [x] 기상청 순포해변 도착 무렵 예보에서 구름 많음·24℃·강수확률 20%·풍속 2.9m/s와
  5km 격자 캐시 1건을 확인했다.
- [x] 강릉시 공영주차장 원천 170건 전체 순회, 유효 좌표 167건 저장. 강릉항회센터
  단건 API에서 반경 1km 이내 3곳과 직선거리·요금·운영시간을 확인했다.
- [x] Backend 215/215, 프로젝트 검사 118개, Android 30 suites 145/145,
  lint 오류 0·경고 44, `assembleDebug` 성공.
...
## 18. 2026-09-08 목적지 검색·지원 지역 처리

- [x] 목적지 검색어의 강제 `강원` 접두어를 제거하고 Kakao Local에 원문을 전달한다.
- [x] 최대 10개 결과를 표시하고 주소가 확인된 강원 결과를 먼저 배치한다.
- [x] 강원 밖 결과는 숨기지 않고 `현재 지원 지역 아님`을 표시하며, 눌러도 목적지로
  선택하지 않고 지속 안내를 보여준다.
- [x] 주소 미확인 결과의 선택과 진행 직전 `/api/region` 좌표 재검증을 유지한다.
- [x] Android `0.12.7`(`versionCode 28`) 31 suites 148/148, lint 오류 0·경고 44,
  `assembleDebug`를 통과했다. 프로젝트 검사는 Node.js 24.19.0에서 118개 파일을 통과했다.
- [ ] 실기기에서 강원·강원 밖·주소 미확인 검색 결과, 320dp 폭과 큰 글자 표시를 확인한다.

debug APK는 64,500,234바이트이고 SHA-256은
`26560C5382EE2D64002BAA096BC62D0ED2CC6C7B4DE0319D567DF19BDF8FBDB0`이다.
