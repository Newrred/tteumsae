...
| 무장애 여행정보 | `코드 검증·운영 미적용` | 강원 무장애 목록 대상의 주출입구·휠체어·화장실·시청각·영유아 안내를 단건 상세에 값이 있을 때만 표시 | 별도 활용신청, migration 010, 실데이터 QA 필요; 필터·추천 미반영 |
| 주변 공영주차장 | `코드 검증·운영 미적용` | 좌표가 있는 강원 공영주차장을 미리 적재하고 장소 반경 1km의 가까운 3곳을 단건·저장 상세에 조건부 표시 | 별도 활용신청, migration 011, Preview·실기기 QA 필요; 직선거리이며 추천 미반영 |
