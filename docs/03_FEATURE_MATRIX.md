...
| 무장애 여행정보 | `운영 코드·DB 반영` | 강원 무장애 목록 대상의 주출입구·휠체어·화장실·시청각·영유아 안내를 단건 상세에 값이 있을 때만 표시 | 활용 승인·실데이터 매칭·실기기 QA 필요; 필터·추천 미반영 |
| 주변 공영주차장 | `운영 코드·DB 반영` | 좌표가 있는 강원 공영주차장을 미리 적재하고 장소 반경 1km의 가까운 3곳을 단건·저장 상세에 조건부 표시 | 전용 키·전체 순회·실기기 QA 필요; 현재 데이터 0건, 직선거리이며 추천 미반영 |
