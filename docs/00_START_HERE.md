...
활용신청은 끝났지만 현재 Production의 기존 `TOUR_API_SERVICE_KEY`가 전용 API에서 403을
반환한다. 새 프로젝트 키로 이 환경변수를 다시 등록한 뒤 실데이터 동기화가 필요하다.
...
날씨·주변 공영주차장은 운영 상세 API에서 실제 값을 확인했습니다. 무장애 정보는 기존
TourAPI 운영 키를 새 프로젝트 키로 교체하기 전까지 숨겨집니다.
