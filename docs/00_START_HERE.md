...
`KorWithService2` 무장애 여행정보도 별도 stage와 migration 010으로 코드가 준비돼 있다.
강원도 무장애 목록에 포함된 장소만 `detailWithTour2`를 조회하고, 기존 TourAPI content ID와
일치하는 장소의 정규화 안내만 상세의 `접근성 안내`에 조건부 표시한다. 활용신청·운영 migration·
실데이터 검증은 아직 하지 않았으며, 기본 검색 화면이나 추천 점수에는 영향을 주지 않는다.

전국주차장정보표준데이터도 별도 `stage=parking`과 migration 011로 코드가 준비돼 있다.
좌표가 있는 강원 공영주차장을 서버 전용으로 적재하고, 장소 반경 1km 안의 가까운 3곳만
단건 상세의 `주변 공영주차장`에 조건부 표시한다. 거리는 도보 경로가 아닌 직선거리임을
명시하며 장소 자체의 TourAPI 주차 문구와 합치지 않는다. 활용신청·운영 migration·Preview
실데이터와 실기기 검증은 아직 하지 않았고 추천·지도 핀·입력 단계에는 영향을 주지 않는다.
...
같은 날 주변 공영주차장 코드까지 포함한 최신 로컬 기준도 Android 145/145,
lint 오류 0·경고 44와 debug APK 빌드, Backend 214/214를 통과했습니다. migration 010~011과
새 공급자 키를 적용하지 않았으므로 이 두 상세 보강은 아직 운영 앱에 나타나지 않습니다.
