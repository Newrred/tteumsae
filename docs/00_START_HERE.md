...
활용신청과 Production `TOUR_API_SERVICE_KEY` 갱신·재배포까지 끝났지만 전용 API만 계속
403을 반환한다. 같은 키의 일반 TourAPI intro는 20건을 정상 처리했으므로 앱이나 환경변수
누락이 아니라 공공데이터포털의 무장애 API 권한 활성화 상태를 다시 확인해야 한다.
...
날씨·주변 공영주차장은 운영 상세 API에서 실제 값을 확인했습니다. 무장애 정보는 키 교체와
재배포 뒤에도 전용 API만 403이어서 공공데이터포털 권한이 활성화될 때까지 숨겨집니다.
