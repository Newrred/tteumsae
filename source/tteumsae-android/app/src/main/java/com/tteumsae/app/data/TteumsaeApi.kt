...
class TteumsaeApi(
    ...
    suspend fun searchPlaces(query: String): List<LocationSearchResult> = withContext(Dispatchers.IO) {
        val normalized = if (query.contains("강원")) query else "강원 $query"
    ...
    suspend fun searchPlace(query: String): LocationSearchResult =
        searchPlaces(query).firstOrNull()
    ...
    suspend fun recommendations(criteria: SearchCriteria): List<SafeRecommendation> =
        withContext(Dispatchers.IO) {
            ...
            response.getJSONArray("data").mapObjects { it.toRecommendation() }
        }
