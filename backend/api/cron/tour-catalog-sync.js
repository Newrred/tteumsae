  ...
  return {
    async fetch(request) {
      ...
      try {
        ...
        state = await deps.getState("tour_catalog_delta");
        const now = deps.now();
        ...
        if (!sourceCursor) {
          const fullState = await deps.getState("tour_api");
          const fallback = fullState.last_completed_at ?? new Date(now.getTime() - 86_400_000);
          sourceCursor = fallback;
        }
        ...
        for (let index = 0; index < maxPages; index += 1) {
          ...
          await deps.upsert(activeRows);
          await Promise.all(activeRows.map((place) => deps.resetEnrichment(place.content_id)));
          await Promise.all(
            inactiveRows.map((place) => deps.setActive(place.content_id, false))
          );
        ...
        try {
          const current = state ?? await deps.getState("tour_catalog_delta");
